with open("/tmp/rules_page.tsx", "r") as f:
    content = f.read()

# I will find the part after the Select Dataset Card and wrap the rest in a TabsContent
...
# Wrap the existing rule builder
validation_content = """        <Tabs defaultValue="validation" className="w-full">
          ...
          <TabsContent value="validation">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">""" + parts[1].replace("  return (\n    <Suspense>\n      <RulesContent />\n    </Suspense>\n  );\n}", "") + """          </TabsContent>
...
function DedupBuilder({ datasetId, columns }: { datasetId: string, columns: string[] }) {
  ...
  const saveConfig = async () => {
    ...
    if (dedupRules.length > 0 && Math.abs(totalWeight - 100) > 0.01) {
      setError(`Total bobot harus 100%. Saat ini: ${totalWeight}%`);
      return;
    }
          ...
          <div className="space-y-4">
            ...
            {dedupRules.length === 0 ? (
              <div className="text-center p-8 border border-dashed rounded-md text-muted-foreground">
                Belum ada kolom yang dipilih. Sistem akan menggunakan tebakan otomatis jika kosong.
              </div>
            ) : (
              <div className="space-y-4">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <Select value={rule.method} onValueChange={(v) => {
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="phone">Phone Match (Standardisasi Nomor HP)</SelectItem>
...
new_content = header + validation_content

# We need to make sure Tabs is imported!
# Let's check if Tabs is in the imports.
import_str = 'import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";\n'
if "Tabs," not in new_content:
    # insert it after import { Button }
    new_content = new_content.replace('import { Button } from "@/components/ui/button";', 'import { Button } from "@/components/ui/button";\n' + import_str)
